...
var forms = function forms() {
  var form = document.querySelectorAll('form'),
      inputs = document.querySelectorAll('input'); //получаем элемменты, которые нам понадобятся
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  header.addEventListener('click', function (e) {
    //навешиваем обработчик события на обшую площадь которая соединяет в себе все табы
    var target = e.target;

    if (target && (target.classList.contains(tabSelector.replace(/\./, "")) || //проверяем, что мы действительно кликнули на один из табов
    target.parentNode.classList.contains(tabSelector.replace(/\./, "")))) {
      tab.forEach(function (item, i) {
        //перебираем эти табы и запоминает не только таб но и номер по порядку(i)
        if (target == item || target.parentNode == item) {
          //как только пользователь кликнул на таб который мы перебираем, мы используем его индекс(i)
          hideTabContent();
          showTabContent(i);
        }
      });
